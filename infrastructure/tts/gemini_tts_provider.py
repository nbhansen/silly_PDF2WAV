...
class GeminiTTSProvider(ITimestampedTTSEngine):
    ...
    # Voice personas for audiobook quality
    VOICE_PERSONAS = {
        "narrator": {
        ...
        "emphasis": {
            "voice": "Kore",    # Balanced, natural
            "style": "engaged and emphatic", 
            "rate": "slightly slower"
        },
        "technical": {
            "voice": "Aoede",   # Clear, professional
            "style": "precise and measured",
            "rate": "slower"
        },
        "dialogue": {
            "voice": "Leda",    # Youthful, expressive
            "style": "conversational",
            "rate": "natural"
        }
    }
    
    def __init__(self, model_name: str = "gemini-2.5-flash-preview-tts", api_key: Optional[str] = None):
        if not api_key:
    ...
    def _generate_with_persona(self, text: str, persona: str) -> bytes:
        ...
        voice_config = self.VOICE_PERSONAS.get(persona, self.VOICE_PERSONAS["narrator"])
