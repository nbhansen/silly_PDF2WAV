...
class GeminiTTSProvider(ITimestampedTTSEngine):
        ...
        try:
            async with self.segment_semaphore:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    audio_data = await loop.run_in_executor(executor, self._generate_audio, text_to_speak)
            ...
            return Result.failure(tts_engine_error(f"Async audio generation failed: {e!s}"))
    ...
    def generate_audio_with_timestamps(self, text_to_speak: str) -> Result[tuple[bytes, list[TextSegment]]]:
        ...
        try:
            ...
            try:
                audio_chunks, timing_segments = loop.run_until_complete(self._generate_segments_async(segments))
            finally:
                loop.close()
            ...
            return Result.failure(tts_engine_error(f"Timestamped audio generation failed: {e!s}"))

    async def _generate_segments_async(self, segments: list[dict[str, str]]) -> tuple[list[bytes], list[TextSegment]]:
        ...
        for i, result in enumerate(results):
            ...
            if not isinstance(result, tuple) or len(result) != 2:
                raise Exception(f"Invalid result format from segment {i}")
    ...
    async def _generate_segment_async(self, seg_data: dict[str, str], index: int) -> tuple[bytes, dict[str, str]]:
        """Generate audio for a single segment with async rate limiting."""
        async with self.segment_semaphore:
            # Use thread pool for blocking API call
            loop = asyncio.get_event_loop()
            with concurrent.futures.ThreadPoolExecutor() as executor:
                audio_data = await loop.run_in_executor(executor, self._generate_audio, seg_data["text"])
                    ...
                    config=types.GenerateContentConfig(
                        ...
                        speech_config=types.SpeechConfig(
                            voice_config=types.VoiceConfig(
                                prebuilt_voice_config=types.PrebuiltVoiceConfig(voice_name=self.configured_voice)
                            )
                        ),
                    ),
                )
    ...
    def _split_into_segments(self, text: str) -> list[dict[str, str]]:
        ...
        for sentence in sentences:
            segments.append({"text": sentence, "content_type": "narrative", "type": "narrative"})
                    ...
                    try:
                        with wave.open(chunk_io, "rb") as w:
                            ...
                            if (
                                ...
                                and w.getsampwidth() == sampwidth
                                and w.getframerate() == framerate
                            ):
                                combined_data += w.readframes(w.getnframes())
                                total_frames += w.getnframes()
                    except Exception:
