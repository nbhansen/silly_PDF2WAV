...
def test_page_range_extraction(pdf_path: str, start_page: int = None, end_page: int = None):
    """Test page range extraction functionality"""
    print("="*70)
    ...
    if start_page or end_page:
        print(f"Extracting pages {start_page or 1} to {end_page or pdf_info['total_pages']}")
    else:
        print("Extracting entire PDF")
    
    try:
        extracted_text = extractor.extract(pdf_path, start_page, end_page)
        ...
        # Save to file
        output_filename = f"page_range_test_{start_page or 1}_{end_page or 'end'}.txt"
        with open(output_filename, "w", encoding="utf-8") as f:
            f.write(extracted_text)
        
        print(f"\n💾 Saved extracted text to: {output_filename}")
...
def test_text_cleaning(text: str, api_key: str = None):
    ...
    cleaner = TextCleaner(api_key or "test_key")
    ...
    try:
        cleaned_chunks = cleaner.clean(text)
        
        print(f"\n✅ CLEANING SUCCESSFUL!")
        print(f"   Input length: {len(text):,} characters")
        print(f"   Output chunks: {len(cleaned_chunks)}")
        
        total_output_length = sum(len(chunk) for chunk in cleaned_chunks)
        print(f"   Total output length: {total_output_length:,} characters")
        print(f"   Compression ratio: {((len(text) - total_output_length) / len(text) * 100):.1f}%")
        
        # Show sample of first chunk
        if cleaned_chunks:
            print(f"\n📖 CLEANED TEXT SAMPLE (first chunk, first 300 chars):")
            print("-" * 40)
            print(repr(cleaned_chunks[0][:300]))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python text_processing.py <pdf_path> [start_page] [end_page]")
        print("\nExamples:")
        print("  python text_processing.py document.pdf")
        print("  python text_processing.py document.pdf 3")
        print("  python text_processing.py document.pdf 3 25")
        sys.exit(1)
    ...
    # Test extraction
    extracted_text = test_page_range_extraction(pdf_path, start_page, end_page)
    
    # Test cleaning if extraction worked
    if extracted_text:
        print("\n" + "="*70)
        api_key = input("Enter Gemini API key for cleaning test (or press Enter to skip): ").strip()
        if api_key:
            test_text_cleaning(extracted_text, api_key)
        else:
            print("Skipping cleaning test")