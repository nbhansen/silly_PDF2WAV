...
from domain.config.tts_config import TTSConfig, TTSEngine, PiperConfig
from domain.errors import Result
from domain.models import PDFInfo, TimingMetadata


# === Flask App Test Fixtures ===

@pytest.fixture
def flask_test_config(temp_dir) -> SystemConfig:
...
@pytest.fixture  
def test_app(flask_test_config) -> Generator[Flask, None, None]:
...
class TestEndToEndPDFConversion:
    ...
    def test_pdf_to_audio_with_timing_data(
        ...
        def generate_timed_audio_side_effect(text: str) -> Result[bytes]:
            ...
            fake_timing_data = {
                "total_duration": len(sentences) * 2.5,
                "segments": [
                    {
                        "text": sentence,
                        "start_time": i * 2.5,
    ...
    def test_page_range_processing(
        self, client: FlaskClient, sample_pdf_content: bytes
    ) -> None:
    ...
    def test_read_along_interface_access(
        self, client: FlaskClient, flask_test_config: SystemConfig
    ) -> None:
