...
@pytest.fixture
def mock_pytesseract_image_to_string():
    """Mock pytesseract.image_to_string."""
    with patch('pytesseract.image_to_string') as mock_its:
        yield mock_its

@pytest.fixture
def mock_convert_from_path():
    """Mock pdf2image.convert_from_path."""
    with patch('pdf2image.convert_from_path') as mock_cfp:
        yield mock_cfp

# Tests for get_pdf_info
def test_get_pdf_info_success(ocr_provider, mock_pdfplumber_open):
    """Test successful retrieval of PDF information."""
    mock_pdf = mock_pdfplumber_open
    ...
    mock_pdfplumber_open.assert_called_once_with("dummy.pdf")

def test_get_pdf_info_no_metadata(ocr_provider, mock_pdfplumber_open):
...
def test_get_pdf_info_error_handling(ocr_provider, mock_pdfplumber_open):
    ...
    assert pdf_info.title == "Unknown"
    assert pdf_info.author == "Unknown"
    mock_pdfplumber_open.assert_called_once_with("nonexistent.pdf")

# Tests for extract_text (formerly extract_text_from_pdf)
def test_extract_text_full_pdf_direct_success(ocr_provider, mock_pdfplumber_open):
...
def test_extract_text_full_pdf_ocr_fallback(ocr_provider, mock_pdfplumber_open, mock_convert_from_path, mock_pytesseract_image_to_string):
    """Test OCR fallback for full PDF when direct extraction fails."""
    mock_pdfplumber_open.return_value.__enter__.return_value.pages = [MagicMock()]
    mock_pdfplumber_open.return_value.__enter__.return_value.pages[0].extract_text.return_value = None # Simulate no direct text
    ...
    mock_pytesseract_image_to_string.return_value = "OCR Text from page 1."
    ...
    mock_pdfplumber_open.assert_called_once_with("dummy.pdf") # Called for direct attempt
    mock_convert_from_path.assert_called_once_with("dummy.pdf", dpi=300, grayscale=True, poppler_path=None)
    mock_pytesseract_image_to_string.assert_called_once_with(mock_image, lang='eng')

def test_extract_text_page_range_direct_success(ocr_provider, mock_pdfplumber_open):
...
def test_extract_text_page_range_ocr_fallback(ocr_provider, mock_pdfplumber_open, mock_convert_from_path, mock_pytesseract_image_to_string):
...
def test_extract_text_invalid_pdf_path(ocr_provider, mock_pdfplumber_open):
...
def test_extract_text_empty_pdf(ocr_provider, mock_pdfplumber_open, mock_convert_from_path, mock_pytesseract_image_to_string):
    ...
    mock_pytesseract_image_to_string.return_value = ""

    page_range = PageRange(start_page=1, end_page=1)
    extracted_text = ocr_provider.extract_text("empty.pdf", page_range)
    assert "OCR process yielded no text." in extracted_text
    mock_pdfplumber_open.assert_called_once_with("empty.pdf")

# Tests for validate_page_range
def test_validate_page_range_valid_full_document(ocr_provider, mock_pdfplumber_open):
...
def test_validate_page_range_valid_partial_document(ocr_provider, mock_pdfplumber_open):
...
def test_validate_page_range_start_page_less_than_1(ocr_provider, mock_pdfplumber_open):
...
def test_validate_page_range_start_page_exceeds_total(ocr_provider, mock_pdfplumber_open):
...
def test_validate_page_range_end_page_less_than_1(ocr_provider, mock_pdfplumber_open):
...
def test_validate_page_range_end_page_exceeds_total(ocr_provider, mock_pdfplumber_open):
...
def test_validate_page_range_start_greater_than_end(ocr_provider, mock_pdfplumber_open):
...
def test_validate_page_range_pdf_info_failure(ocr_provider, mock_pdfplumber_open):
