...
from domain.models import (
    ProcessingRequest, PageRange, PDFInfo, TextSegment, 
    TimingMetadata, TimedAudioResult
)
...
def mock_ocr_provider():
    ...
    mock.get_pdf_info.return_value = PDFInfo(
        total_pages=3,
...
def assert_audio_quality():
    ...
    def _assert_audio_quality(audio_data: bytes, min_size: int = 100):
...
def assert_timing_accuracy():
    ...
    def _assert_timing_accuracy(segments: list, tolerance: float = 0.1):
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if hasattr(item, 'fixturenames') and any(
            'real_' in fixture or 'external_' in fixture 
