...
def mock_llm_provider():
    ...
    mock.generate_summary.return_value = "Cleaned text summary."
    return mock

@pytest.fixture
def mock_tts_engine():
    mock = MagicMock(spec=ITTSEngine)
    mock.generate_audio.return_value = b"dummy_audio_bytes"
    return mock
...
@pytest.fixture
def mock_audio_generation_service():
...
class TestPDFProcessingService:
    ...
    def test_successful_end_to_end_processing(
        self, mock_builtin_open, mock_makedirs, pdf_processing_service, dummy_pdf_path
    ):
        ...
        # Verify file system operations (mocked)
        mock_makedirs.assert_called_once_with("audio_outputs", exist_ok=True)
        # The actual file writing is mocked within AudioGenerationService,
        # so we check if open was called for the combined MP3
        # mock_builtin_open.assert_any_call("audio_outputs/combined.mp3", "wb") # This check is for AudioGenerationService's internal call
    ...
    @patch('os.makedirs')
    @patch('builtins.open', new_callable=mock_open)
    def test_text_extraction_failure(self, mock_builtin_open, mock_makedirs, pdf_processing_service, dummy_pdf_path):
    ...
    @patch('os.makedirs')
    @patch('builtins.open', new_callable=mock_open)
    def test_text_cleaning_failure_empty_chunks(self, mock_builtin_open, mock_makedirs, pdf_processing_service, dummy_pdf_path):
    ...
    @patch('os.makedirs')
    @patch('builtins.open', new_callable=mock_open)
    def test_audio_generation_failure_no_files(self, mock_builtin_open, mock_makedirs, pdf_processing_service, dummy_pdf_path):
    ...
    @patch('os.makedirs')
    @patch('builtins.open', new_callable=mock_open)
    def test_processing_with_specific_page_range(self, mock_builtin_open, mock_makedirs, pdf_processing_service, dummy_pdf_path):
        request = ProcessingRequest(
            pdf_path=dummy_pdf_path,
    ...
    @patch('os.makedirs')
    @patch('builtins.open', new_callable=mock_open)
    def test_processing_with_full_document_page_range(self, mock_builtin_open, mock_makedirs, pdf_processing_service, dummy_pdf_path):
        request = ProcessingRequest(
            pdf_path=dummy_pdf_path,
    ...
    def test_validate_page_range_delegation(self, pdf_processing_service, dummy_pdf_path):
        ...
        pdf_processing_service.text_extractor.validate_range.assert_called_once_with(dummy_pdf_path, page_range)

    @patch('os.makedirs')
    @patch('builtins.open', new_callable=mock_open)
    def test_unexpected_exception_handling(self, mock_builtin_open, mock_makedirs, pdf_processing_service, dummy_pdf_path):
