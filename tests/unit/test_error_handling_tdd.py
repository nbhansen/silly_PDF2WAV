...
from domain.errors import (
    ErrorCode, ApplicationError, Result,
    text_extraction_error, audio_generation_error, tts_engine_error,
    llm_provider_error, invalid_page_range_error, configuration_error,
    file_size_error, unsupported_file_type_error
)
...
class TestErrorCodeEnum:
    ...
    def test_error_code_enum_contains_all_expected_codes(self):
        ...
        expected_codes = {
            'FILE_NOT_FOUND', 'TEXT_EXTRACTION_FAILED', 'TEXT_CLEANING_FAILED',
            'AUDIO_GENERATION_FAILED', 'INVALID_PAGE_RANGE', 'CONFIGURATION_ERROR',
            'TTS_ENGINE_ERROR', 'LLM_PROVIDER_ERROR', 'SSML_PROCESSING_ERROR',
            'FILE_SIZE_ERROR', 'UNSUPPORTED_FILE_TYPE', 'UNKNOWN_ERROR'
        }
...
class TestApplicationError:
    ...
    def test_application_error_with_unicode_content(self):
        ...
        assert "ñáéíóú" in error.details
...
class TestResultType:
    ...
    def test_result_from_exception_with_defaults(self):
        ...
        result = Result.from_exception(exception)
    ...
    def test_result_from_exception_with_complex_exception(self):
        ...
        result = Result.from_exception(exception, ErrorCode.CONFIGURATION_ERROR)
    ...
    def test_result_mutually_exclusive_success_failure(self):
        ...
        failure_result = Result.failure(ApplicationError(ErrorCode.UNKNOWN_ERROR, "error"))
        
        # Success result
        assert success_result.is_success and not success_result.is_failure
        
        # Failure result
        assert failure_result.is_failure and not failure_result.is_success
...
class TestErrorHandlingPatterns:
    ...
    def test_error_recovery_pattern(self):
        ...
        def operation_with_fallback() -> Result[str]:
            ...
            primary_result = Result.failure(tts_engine_error("Gemini unavailable"))
            
            if primary_result.is_failure and primary_result.error.retryable:
...
class TestErrorHandlingEdgeCases:
    ...
    def test_result_with_complex_value_types(self):
        ...
        dict_result = Result.success({"key": "value", "nested": {"inner": 42}})
