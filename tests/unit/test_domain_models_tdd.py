...
class TestPDFInfo:
    ...
    def test_pdf_info_with_unicode_content(self):
        ...
        pdf_info = PDFInfo(
            total_pages=5,
            title="Título en Español: José María",
            author="李小明"
        )
...
class TestProcessingResult:
    ...
    def test_successful_result_with_timing_data(self):
        ...
        from domain.models import TimingMetadata, TextSegment
        
        segments = [
            TextSegment("Hello world", 0.0, 1.0, "sentence", 0, 0)
        ]
    ...
    def test_processing_result_retryable_logic(self):
        ...
        from domain.errors import configuration_error, audio_generation_error
...
class TestTimingMetadata:
    ...
    def test_timing_metadata_get_segment_at_exact_boundaries(self):
        ...
        segments = [
            TextSegment("Test segment.", 1.0, 2.0, "sentence", 0, 0)
        ]
        
        metadata = TimingMetadata(3.0, segments, ["audio.wav"])
        
        # At exact start time
        assert metadata.get_segment_at_time(1.0).text == "Test segment."
        # At exact end time
        assert metadata.get_segment_at_time(3.0).text == "Test segment."
