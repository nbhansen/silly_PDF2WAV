...
class TestAudioGenerationService:
    ...
    def test_convert_single_to_mp3_success(self, mock_subprocess_run, mock_getsize, mock_exists, mock_file_open, mock_path_join, mock_makedirs, audio_service):
    ...
    @patch('subprocess.run')
    def test_convert_single_to_mp3_ffmpeg_failure(self, mock_subprocess_run, mock_getsize, mock_exists, mock_file_open, mock_path_join, mock_makedirs, audio_service):
        mock_subprocess_run.return_value = MagicMock(returncode=1, stderr="FFmpeg error")
        mock_exists.side_effect = [True, False] # Input file exists, but output MP3 does not
    ...
    @patch('subprocess.run')
    def test_create_combined_mp3_concat_demuxer_success(self, mock_subprocess_run, mock_getsize, mock_exists, mock_file_open, mock_path_join, mock_makedirs, audio_service):
        mock_subprocess_run.return_value = MagicMock(returncode=0, stderr="")
        mock_exists.side_effect = [True, True, True, True] # All input files exist, then output MP3 exists

        audio_files = ["file1.wav", "file2.wav"]
        combined_mp3 = audio_service._create_combined_mp3(audio_files, "test_output", "/tmp/audio")
        
        mock_subprocess_run.assert_called_once()
        assert "-f" in mock_subprocess_run.call_args[0][0] and "concat" in mock_subprocess_run.call_args[0][0]
        assert combined_mp3 == "test_output_combined.mp3"

    @patch('subprocess.run')
    def test_create_combined_mp3_concat_filter_fallback_success(self, mock_subprocess_run, mock_getsize, mock_exists, mock_file_open, mock_path_join, mock_makedirs, audio_service):
        # Simulate concat demuxer failure, then concat filter success
        mock_subprocess_run.side_effect = [
            MagicMock(returncode=1, stderr="Demuxer failed"), # First call for demuxer
            MagicMock(returncode=0, stderr="") # Second call for filter
        ]
        mock_exists.side_effect = [True, True, True, True, True] # All input files exist, then output MP3 exists

        audio_files = ["file1.wav", "file2.wav"]
        combined_mp3 = audio_service._create_combined_mp3(audio_files, "test_output", "/tmp/audio")
        
        assert mock_subprocess_run.call_count == 2
        assert "-filter_complex" in mock_subprocess_run.call_args[0][0]
        assert combined_mp3 == "test_output_combined.mp3"

    @patch('subprocess.run')
    def test_create_combined_mp3_ffmpeg_not_available(self, mock_subprocess_run, mock_getsize, mock_exists, mock_file_open, mock_path_join, mock_makedirs, audio_service_no_ffmpeg):
        audio_files = ["file1.wav", "file2.wav"]
        combined_mp3 = audio_service_no_ffmpeg._create_combined_mp3(audio_files, "test_output", "/tmp/audio")
    ...
    def test_generate_audio_with_passed_tts_engine(self, mock_getsize, mock_exists, mock_file_open, mock_path_join, mock_makedirs, audio_service):
        mock_passed_engine = MockTTSEngine(output_format="mp3", audio_data=b"passed_engine_data")
        text_chunks = ["Hello, world!"]
        output_name = "test_output"
        output_dir = "/tmp/audio"

        individual_files, combined_mp3 = audio_service.generate_audio(text_chunks, output_name, output_dir, tts_engine=mock_passed_engine)

        mock_file_open.assert_called_once_with(f"{output_dir}/{output_name}_part01.mp3", "wb")
        mock_file_open().write.assert_called_once_with(b"passed_engine_data")
        assert individual_files == [f"{output_name}_part01.mp3"]
        assert combined_mp3 == f"{output_name}_combined.mp3"