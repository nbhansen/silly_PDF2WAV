...
class AudioGenerationCoordinator:
    ...
    def __init__(self, tts_engine: ITTSEngine, file_manager: IFileManager,
        ...
        print(
            f"AudioGenerationCoordinator: Base delay: {self.base_delay}s, FFmpeg: {audio_processor.check_ffmpeg_availability()}")
    ...
    async def generate_audio_async(self, text_chunks: List[str], output_name: str,
        ...
        print(
            f"AudioGenerationCoordinator: Processing {len(valid_chunks)} valid chunks out of {len(text_chunks)} total")
