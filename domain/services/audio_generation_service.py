...
class AudioGenerationService:
    ...
    def _generate_with_async_service(self, text_chunks: List[str], output_filename: str,
        ...
        try:
            ...
            print(
                f"AudioGenerationService: Async generation completed - {len(audio_filenames)} files, timing: {timing_data is not None}")
    ...
    def _generate_with_sync_strategy(self, text_chunks: List[str], output_filename: str,
        ...
        try:
            ...
            print(
                f"AudioGenerationService: Sync generation completed - {len(result.audio_files) if result.audio_files else 0} files, timing: {result.timing_data is not None}")
    ...
    def _create_timing_data_from_audio_files(self, audio_files: List[str], text_chunks: List[str],
        ...
        try:
            ...
            if len(audio_files) != len(all_sentences):
                print(
                    f"AudioGenerationService: Audio file count ({len(audio_files)}) doesn't match sentence count ({len(all_sentences)})")
            ...
            print(
                f"AudioGenerationService: Created timing data with {len(timing_segments)} segments, total duration: {cumulative_time:.2f}s")
